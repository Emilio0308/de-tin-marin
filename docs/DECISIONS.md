...
| 6   | Precio de bundles         | ✅     | **Dinámico, NO persistido.** Envase + Σ(`prices.unit.netPrice` × `units_per_person`); redondeo comercial hacia arriba al step default **S/ 0.50** (#45). Catálogo/cobro: `normalizedLineTotal`; snapshot guarda también `lineTotal` crudo. Sin columna `prices` en `bundles`.                                                                                                                                                                                                                                                                                                                                                    |
...
| 22  | Modelo de bundles         | ✅     | Bundle = plantilla con `quantity` = **cantidad de sorpresas** (default al personalizar) + `container_id` (envase; #30). `bundle_items.units_per_person` = unidades base **por sorpresa** (default de plantilla; admin editable ≥ 1). En wizard ecommerce, con `storeFeatures.enableUnitsPerPerson = true`, el cliente edita `quantityPerUnit` (≥ 1; init desde plantilla); `totalQuantity = quantityPerUnit × line.quantity`. Sin stock (#5), sin `prices` propio; total dinámico (#6). Ecommerce/guest: `line.quantity` (sorpresas) **15–100**, init = `clamp(bundles.quantity, 15, 100)`; admin `quantity >= 1` sin ese rango. |
...
| 45 | Normalización precio sorpresa | ✅ | Tras sumar envase + dulces (`prices.unit.netPrice`, sin campaña en componentes v1), **redondeo comercial hacia arriba** al múltiplo configurable (default **S/ 0.50**) vía `normalizeBundlePrice` en `@de-tin-marin/shared/bundle-price`. Snapshot: `lineTotal` crudo + `normalizedPerSurprisePrice` + `normalizedLineTotal`. Cobro/UI/emails: `getBundleLineChargeableTotal` (fallback legacy). Admin list/form: `total` normalizado + `rawTotal`. Regla 8. Brief: S4-13 |

## Docs sincronizados (2026-08-31 — normalización precio sorpresa)

- DECISIONS **#45** — `normalizeBundlePrice` (step default 0.5, ceil); snapshot dual raw/normalizado; cobro vía `getBundleLineChargeableTotal`
- `@de-tin-marin/shared/bundle-price` — `computeBundlePerSurprisePrice`, `computeBundleTotal.total` = normalizado
- `order-cart` / `build-order-cart` — componentes bundle con `unitNetPrice` (sin campaña); campos `normalizedPerSurprisePrice` / `normalizedLineTotal` en snapshot
- Validaciones: `orderShoppingCartBundleLineSchema`, `bundleListItemSchema` (`rawTotal`), `bundleLinePreviewSchema`
- Servicios: `bundle.service`, `public-catalog.service`, `order-preview`, `guest-order` drift con totales cobrables
- UI: bundle-form/list, order-form/detail, wizard, cart; notificaciones y reportes con línea cobrable
- Docs: Regla **8**, Regla **16** (subtotal cobrable), `pricing.md` § B, `orders.md` § bundle,
  brief [`stages/S4/13-bundle-price-normalization.md`](stages/S4/13-bundle-price-normalization.md);
  roadmap S4-13 ✅; READMEs catalog / bundle-wizard / cart / orders / notifications
...
- `storeFeatures.enableUnitsPerPerson = **true**` — wizard ecommerce permite
  editar unidades base **por sorpresa** (`quantityPerUnit` ≥ 1) por dulce;
  init desde `bundle_items.units_per_person`; flag off → fuerza 1.
- DECISIONS **#22** actualizado (ya no “fija en 1” en storefront).
- Regla **7**, `database.md` `bundle_items`, brief S3A-2 / S3A-0 / S3A-3,
  roadmap S3A-2, `orders.md` ejemplo, READMEs ecommerce + bundle-wizard.

## Docs sincronizados (2026-08-23 — estados in_transit / awaiting_pickup)

- DECISIONS **#42** — transición logística según `fulfillment.method`; shipment
  obligatorio al pasar a `in_transit`
- Migración `00030_order_status_in_transit_awaiting_pickup.sql` — CHECK status +
  cancel RPC acepta `awaiting_pickup` / `in_transit`
- Shared: `ORDER_STATUSES`, `nextLogisticStatus`, `canTransitionOrderStatus(..., method)`
- Admin: transition + upsert shipment fail-fast; panel envío solo en `in_transit`
  (form inline en `ready`→`in_transit`); labels/stepper
- Ecommerce: labels “En camino” / “Listo para recojo” + progress guest
- Regla **14** ampliada (method-aware); Regla 18 cancel ya listaba nuevos status
- Brief [`stages/S4/10-order-status-in-transit-awaiting-pickup.md`](stages/S4/10-order-status-in-transit-awaiting-pickup.md)
  - roadmap S4-10 ✅
- Docs: `orders.md`, `database.md`, README admin orders
