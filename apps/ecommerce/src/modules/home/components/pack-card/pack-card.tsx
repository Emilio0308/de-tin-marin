...
import type { PackCardProps } from "./pack-card.types";

function PackCardImage({
  pack,
  detailHref,
  variant,
}: Pick<PackCardProps, "pack" | "detailHref" | "variant">) {
  const image =
    variant === "featured" ? (
      <Image
        src={pack.imageUrl}
...
export function PackCard({
  ...
  const formattedPrice = formatPrice(pack.price);
  ...
  return (
    ...
    >
      <PackCardImage pack={pack} detailHref={detailHref} variant={variant} />
        ...
        <div className="@[17rem]/content:flex-row @[17rem]/content:items-center @[17rem]/content:justify-between mt-auto flex flex-col gap-3">
          ...
          {detailHref ? (
            <Link
              ...
              className={cn(
                ctaClassName,
                variant === "listing" && "@[17rem]/content:w-auto w-full",
              )}
            >
              {viewComboLabel}
            </Link>
