...
describe("TermsPage", () => {
  it("renderiza hero, secciones y contacto", () => {
    ...
    const emailLinks = screen.getAllByRole("link", { name: TERMS_EMAIL });
    expect(emailLinks.length).toBeGreaterThan(0);
    expect(emailLinks[0]).toHaveAttribute("href", TERMS_EMAIL_HREF);

    const whatsapp = screen.getAllByRole("link", {
      name: TERMS_CONTENT.contact.whatsappDisplay,
    });
    expect(whatsapp[0]).toHaveAttribute("href", TERMS_WHATSAPP_HREF);
  });
});
