-- S4-04: pack_items.unit_quantity + dual qty checks; deduct pack → baseUnits

alter table catalog.pack_items
  add column if not exists unit_quantity int not null default 0;

alter table catalog.pack_items
  drop constraint if exists pack_items_package_quantity_positive;

alter table catalog.pack_items
  add constraint pack_items_package_quantity_nonnegative
  check (package_quantity >= 0);

alter table catalog.pack_items
  drop constraint if exists pack_items_unit_quantity_nonnegative;

alter table catalog.pack_items
  add constraint pack_items_unit_quantity_nonnegative
  check (unit_quantity >= 0);

alter table catalog.pack_items
  drop constraint if exists pack_items_quantity_sum_positive;

alter table catalog.pack_items
  add constraint pack_items_quantity_sum_positive
  check (package_quantity + unit_quantity >= 1);

-- Extend deduct pack branch: totalPackages → presentationQuantity; totalUnits → baseUnits
-- Bundle / product / container branches unchanged from 00016.
