-- S2A fix: product_type unit → solo loose; líneas producto en presentaciones

create or replace function catalog._deduct_unit_product_loose(
  p_loose int,
  p_need int
)
returns int
...
begin
  if p_need <= 0 then
    return greatest(0, p_loose);
  end if;

  if p_loose < p_need then
    return null;
  end if;

  return p_loose - p_need;
end;
$$;
...
revoke all on function catalog._deduct_unit_product_loose(int, int) from public;
